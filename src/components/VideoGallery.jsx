...
function driveViewSrc(src) {
  const id = driveFileId(src)
  if (!id) return src
  return `https://drive.google.com/file/d/${id}/view`
}

function driveStreamCandidates(fileId) {
  if (!fileId) return []
  return [
 ...
 */
function MobileDrivePlayer({ src, title }) {
 ...
 */
function ActiveDrivePlayer({ src, title, soundOn, onToggleSound, expanded = false }) {
  const fileId = driveFileId(src)
  const candidates = driveStreamCandidates(fileId)
  const videoRef = useRef(null)
  const [mode, setMode] = useState(fileId ? 'stream' : 'iframe')
...
function ExpandModal({ video, ratio, index, total, soundOn, onToggleSound, canPrev, canNext, onPrev, onNext, onClose, isMobile }) {
...
export function VideoGallery({ videos, defaultRatio = 'landscape', cardHeight = DEFAULT_CARD_HEIGHT }) {
  ...
  function go(next) {
    setActive((current) => {
      ...
      return Math.max(0, Math.min(videos.length - 1, target))
    })
  }
...
const styles = {
  ...
  mobilePlayFallbackBg: {
    position: 'absolute',
    inset: 0,
    background: 'linear-gradient(160deg, #1a3229 0%, #0b1613 55%, #12251f 100%)',
  },
  mobilePlayScrim: {
    position: 'absolute',
    inset: 0,
    background: 'linear-gradient(180deg, rgba(11,22,19,0.2) 0%, rgba(11,22,19,0.72) 100%)',
  },
  mobilePlayBadge: {
    position: 'relative',
    zIndex: 1,
