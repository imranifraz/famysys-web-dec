...
import { MotionPrefProvider } from './MotionPrefContext.jsx'
import { TransitionPresetProvider } from './TransitionPresetContext.jsx'
import { ViewportProvider } from './ViewportContext.jsx'
import { EASE_LUX, TRANSITIONS } from './motion.js'
import { portfolioCategories } from '../data/content.js'
import logo from '../assets/famysys-logo.png'
...
export function PresentationShell({ slides }) {
  ...
  const goTo = useCallback(
    (next) => {
      ...
      if (slides[clamped].id === 'selected-work') {
  ...
  const preset = TRANSITIONS[presetIndex]
      ...
      <div ref={viewportRef} style={styles.viewport}>
        ...
        >
          <AnimatePresence key={presenceEpoch} custom={direction} initial={false} onExitComplete={unlock}>
            <motion.div
              key={`${slides[index].id}-${navSeq.current}`}
              custom={direction}
            ...
            >
              <MotionPrefProvider reduced={reducedMotion}>
                <TransitionPresetProvider preset={preset} direction={direction}>
                  <ActiveSlide
                    meta={{ index: index + 1, total: slides.length, title: slides[index].title }}
                    active
          ...
          <div style={isMobile ? styles.chromeBottomMobile : styles.chromeBottom}>
            ...
            <div style={styles.chromeBottomRight}>
              ...
              <SlideNavigation
                ...
                onNext={onNext}
                canPrev={
                  index > 0 || (slides[index].id === 'selected-work' && selectedWorkTab > 0)
                }
                canNext={
                  index < slides.length - 1 ||
                  (slides[index].id === 'selected-work' && selectedWorkTab < SELECTED_WORK_TAB_COUNT - 1)
                }
              />
            </div>
          </div>
